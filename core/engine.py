...
class HomeGuardCore:
    ...
    def __init__(self, claude_config: ClaudeConfig, storage_config: StorageConfig,
        ...
        # Cache de cámaras con vehículos conocidos (TTL 60s)
        self._known_vehicle_cameras: set = set()
        self._kv_cache_ts: float = 0.0
        self._kv_cache_ttl: float = 60.0

    async def handle_event(self, event: SecurityEvent):
        presence = getattr(self, "presence", None)
        if presence and not presence.should_process_event(
                getattr(event, "event_type", ""),
                getattr(event, "camera_id", -1)):
            return
        ...
        if self.yolo and event.snapshot:
            ...
            if image is not None:
                ...
                if yolo_result.detected and not yolo_result.needs_claude:
                    ...
                    # ── Vehículo conocido: registrar sin alerta push ──────────
                    import time as _tc
                    if _tc.monotonic() - self._kv_cache_ts > self._kv_cache_ttl:
                        try:
                            self._known_vehicle_cameras = self.db.get_known_vehicle_cameras()
                        except Exception:
                            self._known_vehicle_cameras = set()
                        self._kv_cache_ts = _tc.monotonic()
                    if str(event.camera_id) in self._known_vehicle_cameras:
                        event.ai_alert = False
                        event.ai_alert_reason = None
                        event.ai_description = (
                            f"{yolo_result.label} detectado — vehículo conocido "
                            f"(YOLO {yolo_result.confidence:.0%})"
                        )
                        logger.info(f"[{event.camera_name}] Vehículo conocido — sin alerta push")
                    # ─────────────────────────────────────────────────────────
