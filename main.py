...
from presence import PresenceMonitor, add_presence_routes
...
async def main():
    ...
    # -- Monitor de presencia y armado --
    presence = PresenceMonitor(
        db_path=os.path.expanduser("~/homeguard/data/homeguard.db"),
        auto_arm=True,
        on_arrival=lambda dev: logger.info(
            f"{dev.person_name} llegó a casa ({dev.device_name})"),
        on_departure=lambda dev: logger.info(
            f"{dev.person_name} salió de casa"),
        on_mode_change=lambda old, new, by: logger.info(
            f"Armado: {old} -> {new} ({by})"),
    )
    core.presence = presence
    presence.start()
    logger.info("Monitor de presencia activo")
    ...
    add_presence_routes(app, presence)
    ...
    asyncio.create_task(_daily_snapshot_cleanup(core.db))

    import hashlib as _hl, os as _os
    _default_pin = _os.environ.get("ADMIN_PIN", "123456")
    with core.db._connect() as _conn:
        for _col in ["ALTER TABLE users ADD COLUMN token TEXT",
                     "ALTER TABLE users ADD COLUMN pin_expiry TEXT",
                     "ALTER TABLE users ADD COLUMN last_login TEXT"]:
            try: _conn.execute(_col)
            except Exception: pass
        _n = _conn.execute("SELECT COUNT(*) FROM users WHERE role='admin'").fetchone()[0]
        if _n == 0:
            _conn.execute("""INSERT INTO users (id, name, role, pin_hash, created_at)
                VALUES ('usr_admin', 'Administrador', 'admin', ?, datetime('now'))""",
                (_hl.sha256(_default_pin.encode()).hexdigest(),))
            _conn.commit()
            logger.info(f"[Auth] Admin creado — PIN: {_default_pin}")
            logger.warning("[Auth] Cambia el PIN admin desde el dashboard")
...
async def _daily_snapshot_cleanup(db):
    """Borra snapshots físicos > SNAPSHOT_RETENTION_DAYS días (defecto 7)."""
    import asyncio as _asyncio, os as _os
    retention = int(_os.environ.get("SNAPSHOT_RETENTION_DAYS", "7"))
    logger.info(f"[Cleanup] Retención de snapshots activa: {retention} días")
    while True:
        await _asyncio.sleep(86400)
        try:
            deleted = db.cleanup_old_snapshots(retention_days=retention)
            if deleted:
                logger.info(f"[Cleanup] {deleted} snapshots purgados")
        except Exception as e:
